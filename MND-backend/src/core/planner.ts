...
export class RoutePlanner {
    ...
    private async busToLocalRoute(
        ...
        let minScore = Infinity; // We optimize for Score, not just Time
        ...
        for (const trip of route.trips) {
            ...
            for (let i = fromIndex + 1; i < trip.stops.length; i++) {
                ...
                if (!localPath.found && !usedDM) continue;

                // Calculate bus segment
                const hopsBefore = fromIndex;
                const busHops = i - fromIndex;
                const timeToFrom = hopsBefore * 5; // 5 mins per stop
                ...
                // Weighted score: penalize local transport time (1.5x) to prefer bus travel
