...
import * as favoritesController from './api/favoritesController';
import * as upcomingController from './api/upcomingController';

// Env vars loaded by import 'dotenv/config'
...
// ============================================================================
// NEW MVP Endpoints - Authentication, Favorites, Upcoming Buses
// ============================================================================

// Auth Routes (Magic Link)
app.post('/api/auth/send-link', authController.sendMagicLink);
...
app.get('/api/favorites', authController.requireAuth, favoritesController.getFavorites);
app.post('/api/favorites', authController.requireAuth, favoritesController.createFavorite);
app.put('/api/favorites/:id', authController.requireAuth, favoritesController.updateFavorite);
app.delete('/api/favorites/:id', authController.requireAuth, favoritesController.deleteFavorite);

// Upcoming Buses & Route Details
app.get('/api/buses/upcoming', upcomingController.getUpcomingBuses);
app.get('/api/buses/schedule/:routeId', upcomingController.getRouteSchedule);
app.get('/api/routes/:routeId', upcomingController.getRouteDetails);
...
app.get('/api/info', (req: Request, res: Response) => {
    res.json({
        ...
        endpoints: {
            ...
            nodes: 'GET /api/nodes',
            routesList: 'GET /api/routes/list',
            planRoute: 'GET /api/routes?from=NODE_ID&to=NODE_ID&time=HH:MM',
            
            // Auth (new)
            sendMagicLink: 'POST /api/auth/send-link',
            verifyMagicLink: 'GET /api/auth/verify?token=TOKEN',
            logout: 'POST /api/auth/logout',
            profile: 'GET /api/profile',
            
            // Favorites (new, requires auth)
            ...
            // Buses (new)
            upcomingBuses: 'GET /api/buses/upcoming?from=NODE_ID&limit=5',
            routeSchedule: 'GET /api/buses/schedule/:routeId',
            routeDetails: 'GET /api/routes/:routeId'
        }
    });
});
