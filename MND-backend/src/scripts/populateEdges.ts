...
async function main() {
    ...
    const allEdgesList = [...edges, ...localEdges];

    console.log('\n🔄 Densifying graph with auto-generated local/walk edges...');
    const densifiedEdges: EdgeData[] = [];
    const allNodes = Array.from(nodeMap.values());
    let densifyCalls = 0;

    // Helper to check if edge exists in hardcoded localEdges or bus edges
    const edgeExists = (from: string, to: string) => {
        return allEdgesList.some(e => e.from === from && e.to === to);
    };
    ...
    for (const fromNode of allNodes) {
        for (const toNode of allNodes) {
            ...
            // Skip if manually defined or already covered by bus
            ...
            if (cache[cacheKey]) {
                durationSeconds = cache[cacheKey].durationSeconds;
                distanceMeters = cache[cacheKey].distanceMeters;
            } else {
                const result = await getDistanceMatrix(fromNode.gmaps_address, toNode.gmaps_address);
                if (result) {
                    densifyCalls++;
            ...
            // Rule 1: Walk (<= 1km) - Approx 12 min/km
            if (distanceMeters <= 1000) {
                densifiedEdges.push({
                    from: fromId,
                    to: toId,
                    mode: 'walk',
                    time_min: Math.round((distanceMeters / 1000) * 12),
                    distance_meters: distanceMeters,
                    cost: 0,
                    one_way: false,
                    source: 'densified_walk'
                });
                console.log(`  + [WALK] ${fromNode.name} → ${toNode.name} (${distanceMeters}m)`);
            }

            // Rule 2: Local (<= 5km) - Using optimistic driving time
            if (distanceMeters <= 5000) {
                densifiedEdges.push({
                    from: fromId,
                    to: toId,
                    mode: 'local',
                    time_min: Math.round(durationSeconds / 60),
                    distance_meters: distanceMeters,
                    cost: Math.max(20, Math.ceil(distanceMeters / 1000) * 10),
                    one_way: false,
                    source: 'densified_local'
                });
                console.log(`  + [LOCAL] ${fromNode.name} → ${toNode.name} (${Math.round(distanceMeters / 100) / 10}km)`);
            }
        }
    }

    console.log(`\n✓ Densification complete: Added ${densifiedEdges.length} new edges (API calls: ${densifyCalls})`);

    const allEdges = [...allEdgesList, ...densifiedEdges];
